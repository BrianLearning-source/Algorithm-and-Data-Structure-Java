...
public class Queue {
    ...
    void enqueue(int dt) { //modify so when data overflow, the program will terminated
        if (!isFull()) {
            if (isEmpty()) {
                front = rear = 0;
            } else {
    ...
    int dequeue() { //modify so when data underflow, the program will terminated
        int dt = 0;
        if (!isEmpty()) {
            dt = data[front];
            size--;
            ...
            System.exit(1); // to stop the program
        }
        return dt;
    }
